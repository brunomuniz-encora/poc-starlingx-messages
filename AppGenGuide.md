...
4. Now the ```app_manifest.yaml``` needs to be configured, and this is the most important configuration step, since It's the only file that it will be needed to build the application. The app manifest configuration can be divided in three steps:
...
5. In this stage the fields inside the **appManifestFile-config** will be configured
   ...
   These are the minimum required fields that will need to filled in order to the app-generator work properly.
...
6. In this part of the file, the application ```metadata.yaml``` file will be configured. The app generator automatically creates the _app_name_ and _app_version_ fields from the values put in the **appManifestFile-config** above. Other values may be passed in order to enable some features within starlingX platform. For a better understanding of each of the attributes in this yaml file refer to
...
7. As in the [app manifest configuration](#app-manifest-configuration), fill the required fields ![setup cfg image](BuildGuide/setup-cfg.png).
...
After the main files had been created, the generator will start packaging everything.

1. Firstly it will package every helm-chart, that was given in the `app_manifest`,
into a ``.tgz`` file. Then it will save this files into a folder named charts.
...
The structure of the app inside the tarball will be as follow
...
In order to allow such customization, the generator allows that the user decide
wether to execute the entire generator or only create the files to be modified
to be package later.
...
After creating the above files, the ap generator will end its execution.
...
The static overrides contains all information that is not to be overwritten
...
and the plugins, should be enough for the application to run as expect in the
StarlingX platform. 

With that in mind, it is recommended to check if the `metadata` and the `setup.cfg`
have been created as it should. Particularly, the `setup.cfg` may need careful
attention if the modifications on the plugins file should be reflected on it.
